...
-För att webservern ska starta autmatiskt, behöver jag lägga till 'state: started' under ansible-builtin.service i min playbook. Då startas nginx direkt  
...
När become: true ligger i början av playbooken, betyder det att alla tasks i hela palybooken körs som root automatiskt. Och man slipper skriva det vid varje task. 
...
-För att webservern ska först stoppas och inaktivieras innan den avinstalleras. Om jag hade tagit bort nginx-paketet först, skulle tjänsten fortfarande försöka köra i minnet, vilket kan skapa fel. 
...
- En bra namngivning på tasks ska tydligt beskriva vad som ska vara sant efter att uppgiften körts. Till exempel 'Ensure nginx is installed', vilket gör playboojen lättare att förstå. 